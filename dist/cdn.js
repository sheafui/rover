(() => {
  ...
  var RoverCollection = class {
    ...
    activateNext() {
      ...
      if (!this.navIndex.length)
        return;
      const current = this.activatedValue.value !== null ? this._navPosMap.get(this.activatedValue.value) ?? -1 : -1;
    ...
    activateByKey(key) {
      ...
      if (!this.navIndex.length)
        return;
      const currentPos = this.activatedValue.value !== null ? this._navPosMap.get(this.activatedValue.value) ?? -1 : -1;
